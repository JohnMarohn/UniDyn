...
Given a time-independent Hamiltonian ${\cal H}$ and an initial density operator $\rho(0)$ evolving in the Schr\"{o}dinger represenation, the \VerbFcn{Evolver} algorithms described below compute the density operator at time $t$ by implementing the following unitary transformation
\begin{equation}
\rho(t) 
...
Consider two representative examples. In the first example, the time evolution of transverse spin magnetization in a longitudinal magnetic field is described in the Schr\"{o}dinger representation by the rotation
\begin{equation}
e^{-i \, \omega t \, I_z} \: 
  I_x \: 
  e^{+i \, \omega t \, I_z} 
...
\end{equation}
where $\omega$ is the Larmor frequency and $I_x$, $I_y$, and $I_z$ are the spin angular-momentum operators. 

In the second example, harmonic oscillator's creation operator $a^{\dagger}$ evolves in the Heisenberg representation under the harmonic oscillator's Hamiltonian ${\cal H} = \frac{1}{2}\,\omega\, (a^{\dagger} \, a + a \, a^{\dagger})$ as follows:
 \begin{equation}
 e^{+ i \omega t \frac{1}{2} (a^{\dagger} \, a + a \, a^{\dagger})} 
   ...
   \label{eq:example-ho}
\end{equation}
where $a$ is the annihilation operator and $\omega$ is the harmonic oscillator's resonance frequency.Crucially, Eq.~\ref{eq:example-spin} makes no reference to a spin quantum number $I$, to a matrix dimension, or to any basis for the operators used. The same is true for Eq.~\ref{eq:example-ho}, and, indeed, the Fock space in which the oscillator operators act is infinite dimensional. 
...
The \VerbFcn{Evolver} algorithms obtain closed-form solutions to Eq.~\ref{Eq:Evolve} by extending an approach introduced by Slichter in Section 2.3 of his \emph{Principles of Magnetic Resonance} text \cite{Slichter1990}.
...
Numerous software programs exist for computing the dynamics described by Eq.~\ref{Eq:Evolve} numerically.
...
Unitary transformations like Eq.~\ref{Eq:Evolve} occur in exact effective Hamiltonian theory \cite{Untidt2002jan,Siminovitch2004jan}, 
...
Most work toward solving Eq.~\ref{Eq:Evolve} to date, motivated by applications in magnetic resonance 
...
Vibrations and photons have an infinite Hilbert space and consequently the associated creation and annihilation operators do not have a matrix representation.
Still, evolution of creation and annihilation operators can be studied using a purely symbolic approach.
...
In the following section we illustrate the Slichter procedure by considering example cases.
We show that his procedure works well for harmonic oscillator problems too, including problems involving the coupling of a harmonic oscillator with a two-level system, a minimum model for describing electron transfer in chemical reactions \cite{Mikkelsen1987feb} and radiation-matter interactions in the strong-coupling limit \cite{Jaynes1963jan,Eberly1980may,Greentree2013nov,Larson2024aug}.
...
We have implemented these algorithms as \emph{Mathematica} functions, \VerbFcn{Evolver1} and \VerbFcn{Evolver2}.
This function evaluates Eq.~\ref{Eq:Evolve} given a Hamiltonian ${\cal H}$, an initial operator $\rho(0)$, and relevant commutation relations.