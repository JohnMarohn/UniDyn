...
\myparagraph{Method 4} We now show that the differential equations in each of these four cases can be reduced to a set of coupled first-order differential equations --- a significant simplification.  We would expect the four cases to still require a different number of coupled of equations. This is not the case.  We show that, remarkably, all four cases can be handled by a set of four coupled differential equations.  This is the essential new insight implemented here in the \VerbFcn{Evolver} algorithm. 
